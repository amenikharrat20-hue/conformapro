...
import { fetchEquipements, fetchEquipementStats, deleteEquipement, fetchTypesEquipement } from "@/lib/controles-queries";
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Contrôles Techniques Réglementaires</h1>
          <p className="text-muted-foreground">Gestion des équipements soumis à contrôles périodiques</p>
        </div>
      ...
      <Card>
        <CardHeader><CardTitle>Équipements ({filteredEquipements?.length || 0})</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? <div className="text-center py-8">Chargement...</div> : filteredEquipements && filteredEquipements.length > 0 ? (
            <Table>
              <TableHeader>
                <TableRow>
      ...
      <AlertDialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Cette action supprimera l'équipement et son historique.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Annuler</AlertDialogCancel>
            <AlertDialogAction onClick={() => { if (equipementToDelete) { deleteMutation.mutate(equipementToDelete); setDeleteConfirmOpen(false); } }} className="bg-destructive">Supprimer</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
